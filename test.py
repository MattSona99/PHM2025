...
if not os.path.exists(os.path.join(cuda_path, "bin", "nvcc.exe")):
    # print("ERRORE CRITICO: Il drive Z: non è montato.") # Commentato per non spammare se sei su CPU
    pass
    ...
    if 'extra_cuda_cflags' in kwargs and kwargs['extra_cuda_cflags']:
        ...
        for flag in kwargs['extra_cuda_cflags']:
            if '-Xptxas' in flag and '-O3' in flag:
                new_flags.append('-Xptxas'); new_flags.append('-O3')
            elif flag.strip() == '-O3': new_flags.append('-O3')
            else: new_flags.append(flag)
        kwargs['extra_cuda_cflags'] = new_flags

    if 'extra_ldflags' in kwargs and kwargs['extra_ldflags']:
        new_ldflags = []
        for flag in kwargs['extra_ldflags']:
            if flag.startswith('-L'): new_ldflags.append(f"/LIBPATH:{cuda_path}lib\\x64")
            elif flag.startswith('-l'):
                lib_name = flag[2:]
                if lib_name == 'cublas': new_ldflags.append('cublas.lib')
                elif lib_name == 'cudart': new_ldflags.append('cudart.lib')
                else: new_ldflags.append(f"{lib_name}.lib")
            else: new_ldflags.append(flag)
...
# =============================================================================
def get_inference_config(model_type, mode='test'):
    ...
    # Logica di selezione cartelle/file in base alla modalità
    if mode == 'test':
        target_folder = 'data/raw/test/'
        output_filename = 'submission.csv'
    elif mode == 'val':
        target_folder = 'data/raw/val/'
        output_filename = 'validation_submission.csv'
    else:
        raise ValueError(f"Modalità {mode} non supportata. Usa 'test' o 'val'.")
    ...
    return {
        ...
        'output_file': f"data/results/{model_type}/{output_filename}",
...
# =============================================================================
def process_batch():
    # Passiamo la modalità configurata
    cfg = get_inference_config(MODEL_TO_TEST, mode=INFERENCE_MODE)
    print(f"--- CONFIGURAZIONE: {cfg['model_type'].upper()} su {cfg['test_folder']} ---")
    ...
    if all_results:
        ...
        print(f"\n--- SUCCESS ---")
        print(f"File salvato in: {cfg['output_file']}")
        print("Esempio righe:", df_res.head(3).values)
    else:
